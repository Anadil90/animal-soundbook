...
### Pre-requisites ###
To view the project on your local machine, you will need vs code on your computer. The link to download vs code for your operating system platform is given here: [vs code] (https://code.visualstudio.com/). Other than that, you will need a web browser such as Google Chrome, Mozilla Firefox, or Microsoft Edge to launch the website locally on your machine. As this is a static website without interactive javascript elements, it is possible to  directly run the website by double clicking on the index.html file without the need for further dependencies. However, the recommended way to live view the website while working on the codebase is to use the extention live server by Ritwick Dey. You can install the extention by going to the extentions menu on the left side (the icon with the four square tiles with one broken off) and then searching for it on the search bar. The extention will show up first when you search for it on the extentions menu. This extention allows you to automatically reload the integrated browser on vs code when the code changes, allowing for a smoother workflow. In order to stage and commit files on the local machine from the command line, you will need git installed on your machine. The link to install git based on your platfrom can be found below.
...
To install git on linux, you will need to do so from the package management tool. This is done using apt, from the command line. At first it is a good idea to update the dependencies using the command sudo apt-get update on the terminal. Then, to install git, you will need to run the command sudo apt-get install git-all.  

### Cloning and getting ready to contribute ###
To start working on the project for further improvements (that you may have in mind), you will need to follow a few steps, which are outlined below.
1. Fork the respository - This means that you will simple be creating your own copy of the repository under your own user name, so that you can freely make changes without affecting the main project base. You can do this by clicking on the fork icon located on the top-right corner of the project respository page. A new copy of the repository will then be created under your user name, and will look like the example below:
https://github.com/your-username/animal-soundbook
2. Clone the respository on your local machine - Now you will make a clone of the forked repository for your computer, where you will make the neccessary changes to the codebase. You will do so by 
To do so, you will need to enter the following command on your terminal in vs code:  git clone https://github.com/Anadil90/animal-soundbook.git 
Upo doing so, you will also need to navigate to the newly created project directory on your system by typing in cd animal-soundbook. And in addition, you will need to verify that the repository has been cloned properly with the command: git remote -v.
3. Create a seperate new branch - You will need to create a seperate branch to make your changes and avoid making changes to the main branch. This is done to steer clear of pitfalls that may occur during the contributing stage, and avoid applying the undesired changes to the main branch. You will be creating a new branch with the command git checkout -b branch-name, where branch-name is the specific name for the new branch you will be creating. It is recommended that you give a meaningful name to the branch you will be working on, as in something that reflects the changes you have in mind. This could be something like for example; soundbook-addition. 
4. Make the neccessary changes and check the status of the files - Now you make the changes where you think is neccessary and then check which files are tracked and untracked. When you are finished with your contributions, you will then get ready to stage the files for commiting to the remote repository you cloned. You can check the status of the files with the command git status.
5. Staging - After your changes have been made, you will need to add the files to be tracked for commiting. This is done with the command: git add . 
6. Commiting and pushing - After the files have been added with git add, you will needd to commit those changes. This is done with the command: git commit -m "your commit message with the changes", where the changes performed are specified within the quotations. 
7. Push changes to the forked respository - Now you will push the newly created branch with the changes to the github fork you created with the command git push origin branch-name, where branch-name is the name for the branch that you just created.
8. Creating the pull request for the changes - Once the branch is pushed with the changes, you should see a button with the name "compare and pull request" on the repository page. Click the button and a prompt will pop up letting you add the details of the pull request. You should add a relevant title of the changes performed, along with a description of what you have done. When you are done, click create pull request to finalize the process and for your changes to be reviewed. If the changes are approved, they will be merged into the main branch of the repository, making them a part of the codebase. If you are requested to makes some changes, then you will need to do so, and then again stage, commit, and push the changes with git push origin branch-name.
...
This user story involved at one point implementing a minimum amount of javascript of a few lines in order to play the sounds. This actually proved to be counteractive, as the high level of javascript knowledge required to pull it off left a lot of unclear and vague ideas of how  to actually get the job done. The humble audio element, in the end proved to be far more suitable for the task of playing the animal sounds, and was implemented.
